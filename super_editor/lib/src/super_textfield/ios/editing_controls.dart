  ...
  Widget _buildToolbar() {
    ...
    final anchorPosition = selection.isCollapsed
        ...
        : (selection.base.offset <= selection.extent.offset
            ? selection.base
            : selection.extent);
    final anchorOffsetInViewport =
        _textPositionToViewportOffset(anchorPosition);
    final lineHeight = _textLayout.getLineHeightAtPosition(anchorPosition);

    toolbarTopAnchor =
        anchorOffsetInViewport - const Offset(0, gapBetweenToolbarAndContent);
    toolbarBottomAnchor = anchorOffsetInViewport +
        Offset(0, lineHeight) +
